...
pub fn main() -> Nil {
  ...
  io.println("Peek Last4 : " <> cowl.mask_with(key, cowl.Peek(cowl.Last(4), "...")))
  io.println("Peek First4: " <> cowl.mask_with(key, cowl.Peek(cowl.First(4), "***")))
  io.println(
    "Peek Both  : "
  ...
  section("Estrazione")
  let hash = cowl.use_secret(pw, fn(raw) { "len=" <> string.inspect(string.length(raw)) })
  ...
  io.println("from_result Err: " <> string.inspect(cowl.from_result(err_result)))
  let labeled_ok =
